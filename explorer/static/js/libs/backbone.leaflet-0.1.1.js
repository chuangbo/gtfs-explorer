...
(function ( Backbone, _, L ) {
  ...
  var MapView = Leaflet.MapView = function ( options ) {
    ...
    if ( this.collection ) {
      ...
      console.log('init mapview', this.collection)
  ...
  _.extend( MapView.prototype, Backbone.View.prototype, {
    ...
    openPopup: function ( obj, content ) {
      ...
      // FIXME (chuangbo): Must have bugs here. Can recur in this way,
      // Put two layer on the map, click first layer to open the popup,
      // and do not click the map, this will close the popup, just click
      // another layer to open another popup. But oops, this popup display
      // it's own content on the first popup, and not show itself.
      
      // TODO: report this bug to upstream.
      this.map.closePopup()
      console.log('getPopup', layer.getPopup())
      if (layer.getPopup()) {
        layer.unbindPopup()
      }
      layer.bindPopup( '' );
    ...
    _onAdd: function ( model ) {
      ...
      console.log('onadd 2', model.attributes.geometry.coordinates)
